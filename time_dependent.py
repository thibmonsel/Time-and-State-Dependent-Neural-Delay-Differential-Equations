...
if __name__ == "__main__":
    ...
    train_idx, test_idx = jnp.load(
        f"data/time_dependent/train_indices_{seed_train_test_split}.npy"
    ), jnp.load(f"data/time_dependent/test_indices_{seed_train_test_split}.npy")
    ...
    if args.model == "anode":
        ...
        y0_anode = y0 = jnp.hstack(
            [ystest[:, 0], jnp.zeros((ystest.shape[0], augmented_dim))]
        )
        anode_ypred, _ = jax.vmap(model, (None, 0))(ts, y0_anode)
        jnp.save(
            default_dir + "/anode/ys_test_pred.npy",
            jnp.asarray(anode_ypred),
        )
