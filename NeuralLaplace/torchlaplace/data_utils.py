...
def basic_collate_fn(
    ...
    data_dict = split_and_subsample_batch(
        ...
        history_nb_time_step=history_nb_time_step
    )
    return data_dict
...
def split_and_subsample_batch(
    data_dict, extrap=False, data="", data_type="train", observe_step=1, predict_step=1,history_nb_time_step=0
):
    if data_type == "train":
        # Training set
        if extrap:
...
def split_data_extrap(data_dict, dataset="", observe_step=1, predict_step=1, history_nb_time_step=0):
    if history_nb_time_step == 0 :
        n_observed_tp = data_dict["data"].size(1) // 2
    else : 
        n_observed_tp = history_nb_time_step 
    if dataset == "hopper":
    ...
    if ("mask" in data_dict) and (data_dict["mask"] is not None):
        ...
        split_dict["mask_predicted_data"] = data_dict["mask"][:, n_observed_tp-1:].clone()
