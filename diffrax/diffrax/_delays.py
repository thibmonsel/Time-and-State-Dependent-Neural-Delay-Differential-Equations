...
from ._custom_types import VF, IntScalarLike, RealScalarLike
...
# class RunForNSteps(optx.AbstractFixedPointSolver):
#     rtol: float
#     atol: float
...
#         return jnp.array(0), self.solver.init(
#             fn, y, args, options, f_struct, aux_struct, tags
#         )

#     def step(self, fn, y, args, options, state, tags):
#         new_y, new_state, aux = self.solver.step(fn, y, args, options, state, tags)
#         return new_y, (state[0] + 1, new_state), aux

#     def terminate(self, fn, y, args, options, state, tags):
#         step, _ = state
#         return (step >= self.num_steps | self.implicit_step), optx.RESULTS.successful
...
#         return state.steps >= self.max_steps, RESULTS.successful
...
# class ModifiedBestSoFarFixedPoint(BestSoFarFixedPoint):  # pyright: ignore
...
#         return self.solver.implicit_step
...
#         num_steps=1,
#         implicit_step=implicit_step,
#         solver=optx.FixedPointIteration(delays.rtol, delays.atol),
#     )
#     # solv = FixedPointIteration(rtol=delays.rtol, atol=delays.atol, norm=rms_norm)
#     nonlinear_args = (
#         terms,
#         implicit_step,
#         dense_interp,
...
#         args,
#     )
#     # jax.debug.print("tprev {} tnext {}", state.tprev, state.tnext)
#     sol = fixed_point(
#         fn,
#         solv,
#         init_guess,
...
#     # jax.debug.print("successful {} {}", sol.result == RESULTS.successful, sol.stats)
